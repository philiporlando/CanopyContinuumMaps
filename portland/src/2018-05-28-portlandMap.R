...
# 2018-05-28
# leaflet map for site selection in Portland, Oregon
...
portl_airnow <- subset(pm25_master_daily, State.Name == "Oregon")

portl_sites <- portl_airnow %>% distinct(Site.Number, .keep_all = TRUE)

portl_coords <- portl_sites[, c(7,6)] # lon, lat

portl_points <- SpatialPointsDataFrame(coords = portl_coords
                                       ,data = portl_sites
...
# reading in census data for Oregon
acs <- get_acs(year = 2016 # unquote on linux
               ...
               ,county = c("Multnomah"
                           ...
                           ,"Columbia"
                           ,"Yamhill"
               ) # counties taken from wiki portl metro area page
               ,key = KEY
               ,output = "wide"
               ,geometry = TRUE
)
...
# reading in census data for Oregon
# acs_or <- get_acs(year = 2016 # unquote on linux
#                ,geography = "block group" 
#                ,variables = "B00001_001E" # unweighted total population
#                ,state = "OR"
#                ,county = c("Multnomah"
...
# reading in census data for Washington 
# acs_wa <- get_acs(year = 2016 # unquote on linux
#                   ,geography = "block group" 
#                   ,variables = "B00001_001E" # unweighted total population
#                   ,state = "WA"
#                   ,county = c("Clark"
#                               ,"Skamania"
#                   ) # counties taken from wiki metro page
#                   ,key = KEY
#                   ,output = "wide"
#                   ,geometry = TRUE
# )

# # combine the two states census data
# acs <- rbind(acs_or, acs_wa)

# reproject into meters
acs <- st_transform(acs, epsg_26910)

# reading in shapefiles for the entire US
urban_areas <- readOGR(dsn = "./data/tigerline/tl_2017_us_uac10.shp")

# include all cities within metro area!
portl <- subset(urban_areas, str_detect(NAME10, "Portland, OR"))

# reprojecting into meters
portl <- spTransform(portl, CRSobj = CRS(epsg_26910))

# determine the bounding box of the urban area
bb <- bbox(portl)
...
grid <- grid_poly[portl, ]

# convert to sf object
grid <- st_as_sf(grid)
portl <- st_as_sf(portl)

# find the existing purpleair sensors that intersect with our urban area 
pa_points <- st_transform(pa_points, crs = st_crs(portl))

# figure out how to use regex to exclude b sensors...
pa_points <- pa_points[portl, ] %>% distinct(geometry, .keep_all = TRUE) # cant get rid of the B sensors
pa_points

# convert back to wgs843 for leaflet map
...
labels_airnow <- sprintf(
  paste0("<strong>EPA AirData: </strong><br/>"
         ,portl_points@data$Local.Site.Name
  )
) %>% 
  lapply(HTML)
...
#airnow_pop <- paste0("<b>Site Name:</b> ", portl_points@data$Local.Site.Name)
...
m2 = leaflet() %>%
  ...
  addCircleMarkers(data = portl_points
                   ,fillColor = "blue"
                   ,color = "blue"
                   ,weight = 1.5
                   ,fillOpacity = 0.5
